  ...
  /*  @Bean
   ...
   /* @Bean
    public ConsumerFactory<String, Meeting> meetingConsumerFactory() {
        ...
        config.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, JsonDeserializer.class);
        return new DefaultKafkaConsumerFactory<>(config, new StringDeserializer(),
                new JsonDeserializer<>(Meeting.class));
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, Meeting> meetingKafkaListenerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, Meeting> factory = new ConcurrentKafkaListenerContainerFactory<>();
